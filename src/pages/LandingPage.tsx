...
import {
  ...
  AlertTriangle, TrendingUp, Github, ExternalLink
} from "lucide-react";
...
const features = [
  ...
  { icon: Terminal, title: "Real-time Logs", desc: "Monitor your bot's activity with live Heroku console output" },
];
...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    const fetchRepos = async () => {
      const { data } = await supabase.from("featured_repos").select("*").eq("is_active", true).order("deploy_count", { ascending: false });
      if (data) setFeaturedRepos(data as FeaturedRepo[]);
    };
    fetchStats();
    fetchRepos();
  ...
  return (
    <div className="min-h-screen bg-background overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/4 w-[800px] h-[800px] rounded-full bg-primary/3 blur-[200px]" />
        <div className="absolute bottom-0 right-1/4 w-[600px] h-[600px] rounded-full bg-[hsl(var(--info))]/3 blur-[200px]" />
      </div>
      ...
      <section className="pt-36 pb-24 px-4 relative">
        <div className="container mx-auto max-w-4xl text-center relative">
          <ScrollReveal>
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass text-primary text-sm font-medium mb-8">
          ...
          <ScrollReveal delay={100}>
            <h1 className="text-5xl md:text-7xl font-bold tracking-tight leading-[0.95] mb-6">
              Host Your WhatsApp
              <br />
              <span className="text-gradient">Bot in Seconds</span>
            </h1>
          </ScrollReveal>
          ...
          <ScrollReveal delay={400}>
            <div className="mt-16 glass-card-premium rounded-2xl overflow-hidden max-w-2xl mx-auto">
              <div className="flex items-center gap-2 px-5 py-3.5 border-b border-[hsl(0_0%_100%/0.06)]">
        ...
        <div className="container mx-auto max-w-5xl">
          <ScrollReveal>
            <div className="text-center mb-12">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full glass text-primary text-sm font-medium mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {[
              ...
              <ScrollReveal key={s.label} delay={i * 80}>
                <div className="glass-card rounded-2xl p-5 text-center glass-hover">
        ...
        <section className="py-20 px-4">
          <div className="container mx-auto max-w-6xl">
            <ScrollReveal>
              <div className="text-center mb-12">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full glass text-primary text-sm font-medium mb-4">
                  <Github className="w-3.5 h-3.5" /> Most Deployed
                </div>
                <h2 className="text-3xl md:text-4xl font-bold mb-4">Popular Bot Repositories</h2>
                <p className="text-muted-foreground text-lg">Top repos deployed on BOTHOST by the community.</p>
              </div>
            </ScrollReveal>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
              {featuredRepos.map((repo, i) => (
                <ScrollReveal key={repo.id} delay={i * 100}>
                  <div className="glass-card-premium rounded-2xl overflow-hidden glass-hover group">
                    {repo.image_url && (
                      <div className="aspect-video overflow-hidden border-b border-[hsl(0_0%_100%/0.06)]">
                        <img src={repo.image_url} alt={repo.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                      </div>
                    )}
                    <div className="p-5">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <h3 className="font-bold text-lg">{repo.name}</h3>
                        <span className="text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary font-semibold whitespace-nowrap">
                          {repo.deploy_count} deploys
                        </span>
                      </div>
                      {repo.description && (
                        ...
                        {repo.repo_url && (
                          <a href={repo.repo_url} target="_blank" rel="noopener noreferrer">
                            <Button variant="outline" size="sm" className="text-xs">
                              <Github className="w-3 h-3 mr-1" /> View Repo
                            </Button>
                          </a>
                        )}
                        <Link to="/register">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map((f, i) => (
              <ScrollReveal key={f.title} delay={i * 80}>
                <div className="glass-card rounded-2xl p-6 glass-hover group h-full">
                  <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center mb-4 group-hover:bg-primary/20 transition-colors group-hover:shadow-lg group-hover:shadow-primary/10">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {steps.map((s, i) => (
              <ScrollReveal key={s.num} delay={i * 100} direction={i % 2 === 0 ? "left" : "right"}>
                <div className="flex gap-4 glass-card rounded-2xl p-6 glass-hover">
        ...
        <div className="container mx-auto max-w-4xl">
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-3xl mx-auto">
            ...
            <ScrollReveal direction="right">
              <div className="glass-card-premium rounded-2xl p-8 relative h-full glow-sm">
                <div className="absolute -top-3 right-6 px-3 py-1 rounded-full bg-gradient-to-r from-primary to-primary/70 text-primary-foreground text-xs font-semibold shadow-lg shadow-primary/20">RECOMMENDED</div>
                <div className="text-sm text-primary font-medium mb-2">PRO</div>
                <div className="text-4xl font-bold mb-1">50 GRD<span className="text-lg text-muted-foreground font-normal">/deploy</span></div>
      ...
      <section id="offers" className="py-24 px-4">
        <div className="container mx-auto max-w-5xl">
          <ScrollReveal>
            <div className="text-center mb-16">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full glass text-warning text-sm font-medium mb-4">
          ...
          <div className="grid md:grid-cols-3 gap-5">
            {offers.map((o, i) => (
              <ScrollReveal key={o.title} delay={i * 100}>
                <div className="glass-card-premium rounded-2xl p-6 glass-hover relative overflow-hidden h-full">
      ...
      <section id="payment" className="py-24 px-4">
        <div className="container mx-auto max-w-3xl">
          ...
          <ScrollReveal delay={100}>
            <div className="glass-card-premium rounded-2xl p-8 space-y-6">
      ...
      {/* Footer */}
      <footer className="border-t border-[hsl(0_0%_100%/0.06)] py-12 px-4">
        <div className="container mx-auto max-w-6xl">
